...
class Word2VecDatasetBuilder(object):
    ...
    :param arch: architecture, skip_gram or cbow
    :type arch: str
    :param algm: algorithm for choosing negative samples
    :type algm: str
    :param epochs: number of times the dataset is iterated.
    :type epochs: int
    :param batch_size: the returned tensors in `get_tensor_dict` have shapes [batch_size, :].
    :type batch_size: int
    :param window_size: num of words on the left or right side of target word within a window.
    ...
    def _build_binary_tree(self, unigram_counts):
        ...
        :param unigram_counts: list of holding word counts. Index of each entry is the same as the word index into the vocabulary.
        :type unigram_counts: List[int]
        :returns: an int numpy array of shape [vocab_size, 2*max_depth+1] where each row holds the codes \
            (0-1 binary values) padded to `max_depth`, and points (non-leaf node indices) padded to `max_depth`,\
            of each vocabulary word. The last entry is the true length of code and point (<= `max_depth`).
        """
...
def subsample(indices, keep_probs):
    """Filters out-of-vocabulary words and then applies subsampling on words in a sentence.

    Words with high frequencies have lower keep probs.

    :param indices: the word indices within a sentence.
    :type indices: rank-1 int tensor
    :param keep_probs: the prob to drop each vocabulary word.
    :type keep_probs: rank-1 float tensor
    :returns: the word indices within a sentence after subsampling.
    :rtype: rank-1 int tensor
    """
...
def generate_instances(
    ...
    def per_target_fn(index, init_array):
        ...
        if arch == 'skip_gram':
            ...
            raise ValueError(f'architecture must be skip_gram or cbow but was {arch}.')
