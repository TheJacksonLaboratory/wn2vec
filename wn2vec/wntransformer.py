# when the pubmed_cr has 2 columns
import nltk
...
class WordNetTransformer:
    ...
    def __init__(self, marea_file, output_file) -> None:
        ...
        with open(marea_file) as f:
            for line in f:
                ...
                payload = columns[1]  #columns[0]: pmid, columns[1] abstract text
                words = payload.split()
                for w in words:
                    self._counter[w] += 1
    ...
    def transform(self, line_abstract: str, _dict) -> str:
        ...
        if len(columns) != 2:
            raise ValueError(f'Malformed marea line: {line}')
        abst_list = columns[1].split()  # columns[0]: pmid, columns[1] abstract text
